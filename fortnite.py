...
print(f'  ')
print(color.CYAN + f'   Thanks For Using Wslt Lobby Bot!')
print(color.CYAN + f'   If You Would Like To Support Me Consider Using Code Wslt')
print(color.CYAN + f'   Credits To: Dances For The Idea')
print(color.CYAN + f'   If You Need Any Help Check out My Discord: https://discord.gg/MpJVfFg')
print(color.CYAN + f'   To Get Your Exchange Code Vist: https://bst.gg/0qt')
print(color.CYAN + f'   Everything Is Done Loading! Have fun :)')
print(f'  ')
...
async def event_friend_message(message):
    ...
    if "help" in args[0].lower():
        if message.author.display_name in data['BlockList']:
            ...
            await message.reply('!extra !skin <skin name> !backpack <backpack name> !emote <emote name> !pickaxe <pickaxe name> !pet <pet name> !emoji <emoji name> !variants <CID> <style type> <integer> !contrail <contrail name> !purpleskull !pinkghoul !Mintyelf !purpleportal !checkeredrenegade !banner <icon> <colour> <level>')
            await message.reply('!stop !help !point <PICKAXE_ID> !ready !unready/!sitin !sitout !bp <level> <xp boost> <friend xp boost> !echo <message> !status <text> !leave !kick <username> !platform <platform>')


    if "extra" in args[0].lower():
        if message.author.display_name in data['BlockList']:
            await message.reply("You don't have access to this command!")
        else:
            await message.reply('!season1 !season2 !season3 !rareskins !copy !hologram !kpop')
    ...
    if "!leave" in args[0].lower():
        if message.author.display_name in data['FullAccess']:
            ...
            delay.sleep(2)
            await client.user.party.me.set_emote('EID_Cry')
            delay.sleep(2)
            await client.user.party.me.leave()
            await message.reply('Good Bye Daddy')
    ...
    if args[0] == "!id":
    ...
    elif "!season2" in args[0].lower():
        rare_skins = ('CID_039_Athena_Commando_F_Disco', 'CID_039_Athena_Commando_F_Disco', 'CID_035_Athena_Commando_M_Medieval', 'CID_035_Athena_Commando_M_Medieval')
        await message.reply('Showing all Season2 Skins Now.')
        ...
        await message.reply('Skin set to Blue Squire!')
        await asyncio.sleep(2)
        ...
        await message.reply('Skin set to Royal Knight!')
        await asyncio.sleep(2)
    ...
    elif "!season3" in args[0].lower():
        rare_skins = ('CID_081_Athena_Commando_F_Space', 'CID_081_Athena_Commando_F_Space', 'CID_088_Athena_Commando_M_SpaceBlack', 'CID_088_Athena_Commando_M_SpaceBlack','CID_083_Athena_Commando_F_Tactical','CID_083_Athena_Commando_F_Tactical','CID_084_Athena_Commando_M_Assassin','CID_084_Athena_Commando_M_Assassin')
        await message.reply('Showing all Season3 Skins Now.')
